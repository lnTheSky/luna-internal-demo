...
namespace Utils
{
    ...
    inline void NormalizeVector(Vector& a)
    {
        if (a.IsZero()) return;
    ...
    inline float GetFovRadius(const float& hFOV, const float& sw, const float& sh, const float& angle)
    {
        float vFOV = 2 * std::atanf( std::tanf(DEG2RAD(hFOV / 2.f)) * 3 / 4.f);
        float hFOV_real = 2 * std::atanf(std::tanf(vFOV / 2.f) * sw / sh);

        float yaw = DEG2RAD(angle);

        float radiusPx = (sw * 0.5f) * std::tanf(yaw) / tanf(hFOV_real * 0.5f);
        return radiusPx;
    }
