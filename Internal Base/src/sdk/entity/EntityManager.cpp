...
EntityManager::EntityManager()
{
    ...
    viewAnglesAddress = client ? client + Offsets::dwViewAngles : 0;
}
...
void EntityManager::Update()
{
    ...
    Vector* viewAnglesPtr = reinterpret_cast<Vector*>(viewAnglesAddress);
    {
        std::unique_lock lock(mutex);
        viewAngles = reinterpret_cast<Vector*>(viewAnglesPtr);
    }
...
Vector* EntityManager::GetViewAngles()
{
    std::shared_lock lock(mutex);
    return viewAngles;
}
