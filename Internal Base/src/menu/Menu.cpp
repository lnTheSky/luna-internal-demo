...
void Menu::Render()
{
    ...
    // ======== Aimbot ========
    CustomCheckbox("Enable Aim", &Globals::aim_enabled);
    ...
    if (ImGui::IsItemClicked())
    {
        bind_popup_pos = ImGui::GetMousePos();
        wait_for_bind = false;
        ImGui::OpenPopup("Aim Bind");
    }

    ImGui::SetNextWindowPos(bind_popup_pos, ImGuiCond_Appearing);
    if (ImGui::BeginPopupModal("Aim Bind", nullptr,
        ImGuiWindowFlags_NoMove |
        ImGuiWindowFlags_NoResize |
        ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::TextUnformatted("Aim Toggle Bind");
        ImGui::Separator();
    ...
    ImGui::Spacing();
    ImGui::SliderFloat("Smooth", &Globals::aim_smooth, 1.f, 50.f);
    ImGui::Spacing();
    
    ImGui::SliderFloat("FOV", &Globals::aim_fov, 0.f, 89.f);
    CustomCheckbox("Aim FOV", &Globals::aim_fov_visible);
    CustomColor("FOV ring color", Globals::aim_fov_color);
